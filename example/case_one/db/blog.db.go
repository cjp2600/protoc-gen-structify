...
// provider: postgres
// protoc-gen-structify: (version=v1.1.43, branch=main, revision=4c3d2b06a298d6516cf76ba59e8e045c38ea39f2), build: (go=go1.23.6, date=2025-10-22T08:13:21+0400)
// protoc: 3.16.0
package db
...
type blogStorages struct {
	...
	prCacheStateStorage PrCacheStateStorage
...
type BlogStorages interface {
	...
	// GetPrCacheStateStorage returns the PrCacheStateStorage store.
	GetPrCacheStateStorage() PrCacheStateStorage
...
func NewBlogStorages(config *Config) (BlogStorages, error) {
	...
	prCacheStateStorageImpl, err := NewPrCacheStateStorage(config)
	if err != nil {
		return nil, fmt.Errorf("failed to create PrCacheStateStorage: %w", err)
	}
	storages.prCacheStateStorage = prCacheStateStorageImpl
...
// GetPrCacheStateStorage returns the PrCacheStateStorage store.
func (c *blogStorages) GetPrCacheStateStorage() PrCacheStateStorage {
	return c.prCacheStateStorage
}
...
// Value implements the driver.Valuer interface for JSON.
func (m *UserCommentMeta) Value() (driver.Value, error) {
	if m == nil {
		m = &UserCommentMeta{}
	}
	return json.Marshal(m)
...
// PostTagsRepeated is a UUID array type for Postgres.
type PostTagsRepeated []string

// NewTagsField returns a new PostTagsRepeated.
func NewTagsField(v []string) PostTagsRepeated {
	return v
}

// Scan implements the sql.Scanner interface for UUID arrays.
func (m *PostTagsRepeated) Scan(src interface{}) error {
	if src == nil {
		*m = nil
		return nil
	}
...
func (m PostTagsRepeated) Get() []string {
	return m
}

func (m *PostTagsRepeated) String() string {
