...
// provider: postgres
// protoc-gen-structify: (version=v1.0.0-37-gf23dd3b, branch=main, revision=f23dd3b9ceee23464c4dbab1b98a195000126127), build: (go=go1.20.7, date=2023-08-18T18:14:22+0300)
// protoc: 3.15.8
package db
...
type UserStorage interface {
	...
	LoadDevice(ctx context.Context, model *User) error
	// LoadSettings loads the Settings relation.
	LoadSettings(ctx context.Context, model *User) error
	// LoadAddresses loads the Addresses relation.
	LoadAddresses(ctx context.Context, model *User) error
}
...
// LoadDevice loads the Device relation.
func (t *userStorage) LoadDevice(ctx context.Context, model *User) error {
	s := NewDeviceStorage(t.db)
	relationModel, err := s.FindOne(ctx, FilterBuilder(DeviceUserIdEq(model.Id)))
	if err != nil {
...
// LoadSettings loads the Settings relation.
func (t *userStorage) LoadSettings(ctx context.Context, model *User) error {
	s := NewSettingStorage(t.db)
	relationModel, err := s.FindOne(ctx, FilterBuilder(SettingUserIdEq(model.Id)))
	if err != nil {
...
// LoadAddresses loads the Addresses relation.
func (t *userStorage) LoadAddresses(ctx context.Context, model *User) error {
	s := NewAddressStorage(t.db)
	relationModels, err := s.FindMany(ctx, FilterBuilder(AddressUserIdEq(model.Id)))
	if err != nil {
...
type SettingStorage interface {
	...
	LoadUser(ctx context.Context, model *Setting) error
}
...
// LoadUser loads the User relation.
func (t *settingStorage) LoadUser(ctx context.Context, model *Setting) error {
	s := NewUserStorage(t.db)
	relationModel, err := s.FindOne(ctx, FilterBuilder(UserIdEq(model.UserId)))
	if err != nil {
...
type AddressStorage interface {
	...
	//
	// LoadUser loads the User relation.
	LoadUser(ctx context.Context, model *Address) error
}
...
// LoadUser loads the User relation.
func (t *addressStorage) LoadUser(ctx context.Context, model *Address) error {
	s := NewUserStorage(t.db)
	relationModel, err := s.FindOne(ctx, FilterBuilder(UserIdEq(model.UserId)))
	if err != nil {
		return fmt.Errorf("failed to find UserStorage: %w", err)
